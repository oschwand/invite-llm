...
Both modes then `POST /team/member_add` ("already in team" errors tolerated → idempotent) and `POST /key/generate` with the team's member budget (`team_member_budget_table.max_budget`) as `max_budget`. Returns `{key, user_id, team_id}` — the plaintext `sk-...` key is shown once by the frontend. All upstream LiteLLM calls use the master key; errors are re-raised as FastAPI `detail` strings the frontend's `extractErrorMessage` understands (502 = upstream failure, 503 = missing master key config).
