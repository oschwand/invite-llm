...
- The **Allowed models** column (`keyModels`) shows a compact stub (first model, truncated at 24 chars, `… +N` when several; "All models" when `models` is null/empty; full list in the tooltip). Clicking opens the models modal (`openModelsModal`/`loadModels`): `GET /model/info` with the session key gives per-model data; each row shows the **mode** (`model_info.mode`: `chat`, `completion`, `embedding`, `image_generation`, `audio_transcription`, `audio_speech`, `rerank`, …) as a badge next to the name, **all non-zero `*cost*` fields of `model_info**` (`modelSummary` + `modelCostParts`: per-token/per-character/per-audio-token costs are shown per million (`/Mtok`, `/Mchar`, `/M audiotok`), per-image/per-second/per-request as-is (`/image`, `/s`, `/request`); known fields get short labels like `in`/`out`/`cache read`/`cache write`, unknown ones a prettified field name), plus context = `max_input_tokens` falling back to `max_tokens` (`formatContext` → `128k`). Restricted keys filter the list to `key.models`; if `/model/info` fails it falls back to `GET /v1/models` (names only, no pricing).
