...
- **Invite link format: `{app origin}/{team_id}/{invite_code}`** (`window.location.origin` + path; e.g. `http://localhost:5173/<team-uuid>/<8-char-code>`). Built by `teamInviteLink(team)` for the Teams table and inline in `createTeam()` (using the `team_id` from the `POST /team/new` response) for the post-creation panel. The SPA has no router yet — nothing serves this path; redeeming it is a future feature.
